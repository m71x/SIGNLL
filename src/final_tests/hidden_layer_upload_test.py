...
# --- Helper: Upload file to GCS (ADDED CORE ID TO LOGS) ---
def upload_file_to_gcs(bucket_name, local_path, gcs_blob_path):
    ...
    print(f"[Core {core_id}] Starting GCS upload for {local_path}...", flush=True) # DEBUG START
    ...
    try:
        ...
        print(f"[Core {core_id}] ✅ Uploaded: {local_path} → gs://{bucket_name}/{gcs_blob_path}", flush=True) # SUCCESS LOG
        success = True
    except Exception as e:
        print(f"[Core {core_id}] ❌ Upload failed for {local_path}: {e}", flush=True) # FAILURE LOG
    return success # Return success flag


# --- Helper: Download file from GCS (ADDED CORE ID TO LOGS) ---
def download_file_from_gcs(bucket_name, gcs_blob_path, local_path):
    """Downloads a file from GCS to local storage."""
    core_id = xm.get_ordinal()
    print(f"[Core {core_id}] Starting GCS download for {gcs_blob_path}...", flush=True) # DEBUG START
    try:
        client = storage.Client()
        bucket = client.bucket(bucket_name)
        blob = bucket.blob(gcs_blob_path)
        ...
        print(f"[Core {core_id}] ✅ Downloaded: gs://{bucket_name}/{gcs_blob_path} → {local_path}", flush=True) # SUCCESS LOG
        return True
    except Exception as e:
        print(f"[Core {core_id}] ❌ Download failed for {gcs_blob_path}: {e}", flush=True) # FAILURE LOG
        return False
...
def _mp_fn(index):
    ...
    with torch.no_grad():
        ...
        # FIXED: Cast BFloat16 to Float32 before moving to CPU and converting to numpy
        # Use .cpu() for cleaner host transfer syntax
    ...
    print(f"[Core {core_id}] Saved NumPy data locally to {local_path}", flush=True) # DEBUG LOG
    ...
    # --- END TIMING HERE ---
    end_time = time.time() # <-- END TIME RECORDED HERE
    
    if upload_success:
        duration = end_time - start_time
        print(f"[Core {core_id}] ⏱️ Time (Model Output to GCS Upload Success): {duration:.4f} seconds", flush=True) # <-- PRINT DURATION
    
    print(f"[Core {core_id}] Finished GCS Upload attempt.", flush=True) # DEBUG LOG
    ...
    print(f"[Core {core_id}] Finished GCS Download attempt.", flush=True) # DEBUG LOG

    try:
        reloaded = np.load(verify_path)
        arrays = [reloaded[k] for k in reloaded.files]
        print(f"[Core {core_id}] 🔁 Verified {len(arrays)} arrays. Last array shape: {arrays[-1].shape}", flush=True)
    except Exception as e:
        print(f"[Core {core_id}] ❌ Verification failed: {e}", flush=True)

    # The final rendezvous is left here, but the detailed logging will confirm 
    # if a core is hanging on the GCS calls just before this.
    xm.rendezvous("done") 
