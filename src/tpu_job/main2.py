...
def main_worker(index, local_base):
    ...
    if TEST_MODE_SINGLE_CHUNK:
        log(core_id, "🧪 TEST MODE: Will process only ONE chunk and exit.")
    ...
    # *** TEST MODE: Track if we've uploaded one chunk ***
    chunk_uploaded = False

    # Iterate in batches
    while current_index < total_samples:
        # *** TEST MODE: Exit after first chunk upload ***
        if TEST_MODE_SINGLE_CHUNK and chunk_uploaded:
            log(core_id, "🧪 TEST MODE: One chunk uploaded successfully. Exiting early.")
            break
        ...
        if processed_since_last_save >= IO_ACCUMULATION_THRESHOLD:
            ...
            try:
                ...
                # *** TEST MODE: Mark that we've uploaded a chunk ***
                chunk_uploaded = True
    ...
    # *** TEST MODE: Skip final chunk if we already uploaded one ***
    if accumulated_cls and not (TEST_MODE_SINGLE_CHUNK and chunk_uploaded):
        ...
        try:
            ...
            # *** TEST MODE: Mark chunk uploaded ***
            if TEST_MODE_SINGLE_CHUNK:
                chunk_uploaded = True
                
        except Exception as e:
            log(core_id, f"Failed final upload: {e}")
    elif TEST_MODE_SINGLE_CHUNK and chunk_uploaded:
        log(core_id, "🧪 TEST MODE: Skipping final chunk upload (already uploaded one chunk).")
    ...
    if current_index > start_index:
        try:
            ...
            log(core_id, f"Final checkpoint update: samples_seen set to {current_index}.")
        except Exception as e:
    ...
    if TEST_MODE_SINGLE_CHUNK:
        if chunk_uploaded:
            log(core_id, "🧪 TEST MODE COMPLETE: Successfully uploaded 1 chunk and exited early.")
        else:
            log(core_id, "🧪 TEST MODE COMPLETE: No chunk uploaded (insufficient data or all filtered out).")
...
if __name__ == '__main__':
    ...
    #NEVER SPECIFY Nprocs if you want to use all coresd
    xmp.spawn(main_worker, args=(LOCAL_BASE,)) 
